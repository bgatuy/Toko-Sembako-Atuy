...
export function renderCategories() {
  ...
  const categories = [
    "Semua",
    ...new Set(products.map((p) => p.category || "Lainnya").filter((c) => c)),
  ];
...
export function renderProducts() {
  ...
  products
    .filter((p) => {
      ...
      const matchCat =
        ...
        (p.category || "Lainnya") === currentCategory;
  ...
  `;
      ...
      let imgSrc = "";
      if (p.image) {
        imgSrc = p.image.startsWith("data:")
          ? p.image
    ...
    `;
      ...
      if (p.stock > 0) {
        if (
          (p.unitBig && p.conversion > 1) ||
          (p.unitMedium && p.conversionMedium > 1)
        )
...
export function openUnitSelection(product) {
  ...
  if (product.unitMedium && product.conversionMedium > 1) {
    ...
    document.getElementById("priceUnitMedium").textContent = rupiah(
      product.priceMedium || product.price * product.conversionMedium,
    );
    btnMedium.onclick = () => {
  ...
  document.getElementById("labelUnitBig").textContent =
    product.unitBig || "Dus";
  document.getElementById("priceUnitBig").textContent = rupiah(
    product.priceBig || product.price * product.conversion,
  );
...
export function openProductModal(id = null) {
  ...
  if (id) {
    ...
    document.getElementById("prodCapitalPrice").value = formatNumber(
      p.capitalPrice || 0,
    );
    ...
    document.getElementById("prodPriceBig").value = p.priceBig
      ? formatNumber(p.priceBig)
      : "";
    document.getElementById("prodCapitalPriceBig").value =
      p.capitalPrice && p.conversion
        ? formatNumber(p.capitalPrice * p.conversion)
        : "";
    document.getElementById("prodUnitMedium").value = p.unitMedium || "";
    document.getElementById("prodConversionMedium").value =
      p.conversionMedium || "";
    document.getElementById("prodPriceMedium").value = p.priceMedium
      ? formatNumber(p.priceMedium)
      : "";
    document.getElementById("prodCapitalPriceMedium").value =
      p.capitalPriceMedium ? formatNumber(p.capitalPriceMedium) : "";

    // Setup Preview Gambar
    if (p.image) {
      previewImg.src = p.image.startsWith("data:")
        ? p.image
        : `assets/images/${p.image}`;
...
export function saveProduct() {
  ...
  const conversionMedium = Number(
    document.getElementById("prodConversionMedium").value,
  );
  const priceMedium = getNumber(
    document.getElementById("prodPriceMedium").value,
  );
  const capitalPriceMedium = getNumber(
    document.getElementById("prodCapitalPriceMedium").value,
  );
...
export function saveStock() {
  ...
  if (!amount && type !== "set" && amount !== 0)
    return showAlert("Masukkan jumlah");
