...
export function setPaymentMethod(method) {
  ...
  methods.forEach((m) => {
    ...
    if (m === method) {
      btn.className =
        "bg-primary text-white border border-primary rounded-lg py-2 text-xs font-bold transition";
    } else {
      btn.className =
        "bg-white text-gray-500 border border-gray-200 rounded-lg py-2 text-xs font-bold hover:bg-gray-50 transition";
    }
  });
...
export function openModal() {
  ...
  const cashVal =
    currentPaymentMethod === "cash" ? getNumber(cashInput.value) : calc.total;
  ...
  document.getElementById("rDiscount").textContent =
    "- " + rupiah(calc.discount);
  document.getElementById("rTotal").textContent = rupiah(calc.total);
  document.getElementById("rCash").textContent = rupiah(cashVal);
  document.getElementById("rChange").textContent = rupiah(
    currentPaymentMethod === "cash" ? Math.max(cashVal - calc.total, 0) : 0,
  );
...
export function processCheckout() {
  ...
  const cash =
    currentPaymentMethod === "cash" ? getNumber(cashInput.value) : total;
  const change =
    currentPaymentMethod === "cash" ? Math.max(cash - total, 0) : 0;
...
export function showHeldTransactions() {
  ...
  if (held.length === 0) {
    ...
    held.forEach((t) => {
      const totalItems = Object.values(t.items).reduce(
        (acc, item) => acc + item.qty,
        0,
      );
      const itemsHtml = Object.values(t.items)
        .map(
          (i) => `
      ...
      el.className =
        "bg-white border border-gray-200 rounded-xl p-3 hover:border-primary/50 transition-all shadow-sm";
...
export function restoreHeldTransaction(id) {
  if (Object.keys(cart).length > 0) {
    showConfirm(
      "Keranjang saat ini tidak kosong. Timpa dengan transaksi tersimpan?",
      () => performRestore(id),
      "Timpa",
      "Batal",
    );
