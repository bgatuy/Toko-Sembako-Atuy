...
export function renderHistory(resetPage = false) {
  ...
  paginatedData.forEach((t) => {
    const discountInfo =
      t.discount > 0
        ? `<div class="text-xs text-green-600">Potongan ${rupiah(t.discount)}</div>`
        : "";
...
export function showTransactionDetail(id) {
  ...
  document.getElementById("detailSubtotal").textContent = rupiah(
    t.subtotal || t.total,
  );
...
export function resetHistory() {
  if (transactions.length === 0)
    return showAlert("Riwayat transaksi sudah kosong.");
...
export function exportToExcel() {
  ...
  let csvContent =
    "Tanggal;ID Transaksi;Item;Metode;Subtotal;Diskon;Total;Modal;Laba\n";

  data.forEach((t) => {
    const capital = t.itemsDetails
      ? t.itemsDetails.reduce(
          (sum, i) => sum + (i.capitalPrice || 0) * i.qty,
          0,
        )
      : 0;
