...
export function addToCart(product, unitType = "base") {
  ...
  if (isBig) {
    ...
    capitalPrice =
      product.capitalPriceBig ||
      product.capitalPrice * (product.conversion || 1);
    conversion = product.conversion || 1;
    unitName = product.unitBig || "Dus";
  } else if (isMedium) {
    price =
      product.priceMedium || product.price * (product.conversionMedium || 1);
    capitalPrice =
      product.capitalPriceMedium ||
      product.capitalPrice * (product.conversionMedium || 1);
  ...
  if (totalStockUsed + conversion > product.stock)
    return showAlert("Stok tidak mencukupi!");
...
export function updateQty(cartKey, delta) {
  if (delta > 0) {
    ...
    if (totalStockUsed + item.conversion > product.stock)
      return showAlert("Stok maksimal tercapai!");
  }
...
export function renderCart() {
  ...
  Object.values(cart).forEach((item) => {
    ...
    el.className =
      "bg-gray-50 rounded-xl p-3 flex justify-between items-center";
    el.innerHTML = `
      <div>
